...
class EditorLua
{
	...
	public function new(script:String)
	{
		...
		var status:Dynamic = LuaL.luau_loadsource(lua, script, code);
		var resultStr:Dynamic = Lua.tostring(lua, -1);
		if (resultStr != null && status != 0)
		{
	...
	public function call(event:String, args:Array<Dynamic>):Dynamic
	{
		...
		if (result != null && resultIsAllowed(lua, result))
		{
			/*var resultStr:String = Lua.tostring(lua, result);
				...
				Lua.pop(lua, 1); */
			if (Lua.typename(lua, Lua.type(lua, -1)) == 'string') // name-based: LUA_TSTRING is not enum-stable across Luau revisions
			{
				var error:String = Lua.tostring(lua, -1);
				Lua.pop(lua, 1);
	...
	{ // Makes it ignore warnings
		// Type tags up to LUA_TNUMBER are stable across Luau revisions; string/table are not
		// (newer Luau inserts LUA_TINTEGER), so compare those by VM-reported name.
		var type:Int = Lua.type(leLua, leResult);
		if (type <= Lua.LUA_TNUMBER)
			return true;
		var typeName:String = Lua.typename(leLua, type);
		return typeName == 'string' || typeName == 'table';
	}
	#end
