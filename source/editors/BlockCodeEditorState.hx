...
class BlockCodeEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// 2. Backgrounds (Dark Theme)
		var bg:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, COLOR_BG);
		bg.scrollFactor.set();
		add(bg);

		// Infinite Grid (Subtle)
		var gridBG:FlxSprite = FlxGridOverlay.create(40, 40, FlxG.width * 4, FlxG.height * 4, true, COLOR_GRID_LINE, COLOR_BG);
	...
	{
		// Sidebar Background
		var catStrip = new FlxSprite(0, 0).makeGraphic(70, FlxG.height, COLOR_SIDEBAR_STRIP);
		catStrip.cameras = [camSidebar];
		catStrip.scrollFactor.set(0, 0);
		add(catStrip);

		var sidebarBg = new FlxSprite(70, 0).makeGraphic(250, FlxG.height, COLOR_SIDEBAR_BG);
		sidebarBg.cameras = [camSidebar];
		sidebarBg.scrollFactor.set(0, 0);
		add(sidebarBg);

		var sep = new FlxSprite(319, 0).makeGraphic(1, FlxG.height, COLOR_SEPARATOR);
		...
		// Status Bar
		var statusBar = new FlxSprite(0, FlxG.height - 30).makeGraphic(FlxG.width, 30, COLOR_STATUS_BAR);
		statusBar.cameras = [camHUD];
		add(statusBar);

		statusText = new FlxText(10, FlxG.height - 25, FlxG.width - 20,
		...
		trashCan.makeGraphic(60, 60, COLOR_TRASH);
	...
	{
		var barBg = new FlxSprite(310, 0).makeGraphic(10, FlxG.height, COLOR_SCROLL_TRACK);
		barBg.cameras = [camSidebar];
		barBg.scrollFactor.set(0, 0);
		add(barBg);

		scrollThumb = new FlxSprite(310, 0).makeGraphic(10, 50, COLOR_SCROLL_THUMB);
		...
		categories = [
			{
				...
				blocks: [
					...
					},
					{
						...
						parameters: [
							{name: "n", type: "string", defaultValue: "name"},
							{name: "v1", type: "string", defaultValue: "val1"},
							{name: "v2", type: "string", defaultValue: "val2"}
						]
					},
				]
			},
				...
				blocks: [
					{
						...
						parameters: [{name: "condition", type: "string", defaultValue: "true"}]
					},
					{
						type: "elseif",
						label: "else if",
					...
					},
					{
						...
						parameters: [{name: "seconds", type: "number", defaultValue: 1.0}]
					},
					{
						type: "debugPrint",
						label: "debugPrint",
				...
				blocks: [
					...
					},
					{
						...
						parameters: [
							...
							{name: "b", type: "string", defaultValue: "true"}
						]
					},
					{
						type: "or",
					...
					},
					{
						...
						isReporter: true,
						parameters: [{name: "a", type: "string", defaultValue: "true"}]
					},
					{
						type: "join",
						label: "join",
						...
						parameters: [
							...
							{name: "b", type: "string", defaultValue: "world"}
						]
					},
				]
			},
				...
				blocks: [
					{
						...
						parameters: [
							...
							{name: "val", type: "string", defaultValue: "value"}
						]
					},
					{
						type: "getProperty",
					...
					},
					{
						...
						parameters: [
							{name: "tag", type: "string", defaultValue: "sprite"},
							{name: "image", type: "string", defaultValue: "image"},
							{name: "x", type: "number", defaultValue: 0},
							{name: "y", type: "number", defaultValue: 0}
						]
					},
					...
					},
					{
						...
						parameters: [
							{name: "tag", type: "string", defaultValue: "sprite"},
							{name: "front", type: "string", defaultValue: "false"}
						]
					},
					{
						type: "scaleObject",
					...
					},
					{
						...
						parameters: [
							...
							{name: "cam", type: "string", defaultValue: "hud"}
						]
					},
					{
						type: "screenCenter",
						...
						parameters: [{name: "tag", type: "string", defaultValue: "tag"}]
					},
				]
			},
			{
				...
				blocks: [
					{
						...
						parameters: [
							{name: "sound", type: "string", defaultValue: "scrollMenu"},
							{name: "vol", type: "number", defaultValue: 1.0}
						]
					},
					{
						type: "playMusic",
						...
						parameters: [
							{name: "sound", type: "string", defaultValue: "music"},
							{name: "vol", type: "number", defaultValue: 1.0},
							{name: "loop", type: "string", defaultValue: "true"}
						]
					},
					{
						type: "pauseSound",
						...
						parameters: [{name: "sound", type: "string", defaultValue: "sound"}]
					},
					{
						type: "stopSound",
						label: "stopSound",
				...
				blocks: [
					{
						...
						parameters: [
							{name: "tag", type: "string", defaultValue: "tag"},
							{name: "obj", type: "string", defaultValue: "obj"},
							{name: "val", type: "number", defaultValue: 100},
							{name: "dur", type: "number", defaultValue: 1},
							{name: "ease", type: "string", defaultValue: "linear"}
						]
					},
					{
						type: "doTweenY",
					...
					},
					{
						...
						parameters: [
							{name: "tag", type: "string", defaultValue: "tag"},
							{name: "obj", type: "string", defaultValue: "obj"},
							{name: "val", type: "number", defaultValue: 1},
							{name: "dur", type: "number", defaultValue: 1},
							{name: "ease", type: "string", defaultValue: "linear"}
						]
					},
					{
						...
						parameters: [
							{name: "tag", type: "string", defaultValue: "tag"},
							{name: "cam", type: "string", defaultValue: "game"},
							{name: "val", type: "number", defaultValue: 1},
							{name: "dur", type: "number", defaultValue: 1},
							{name: "ease", type: "string", defaultValue: "linear"}
						]
				...
				blocks: [
					{
						...
						parameters: [{name: "val", type: "number", defaultValue: 1.0}]
					},
					{
						type: "addHealth",
						label: "addHealth",
						...
						parameters: [{name: "val", type: "number", defaultValue: 0.1}]
					},
					{
						type: "cameraShake",
						label: "cameraShake",
						...
						parameters: [
							{name: "cam", type: "string", defaultValue: "game"},
							{name: "int", type: "number", defaultValue: 0.05},
							{name: "dur", type: "number", defaultValue: 0.5}
						]
					},
					{
						type: "cameraFlash",
						...
						parameters: [
							{name: "cam", type: "string", defaultValue: "game"},
							{name: "col", type: "string", defaultValue: "FFFFFF"},
							{name: "dur", type: "number", defaultValue: 0.5}
						]
					},
					{
						...
						parameters: [
							{name: "cam", type: "string", defaultValue: "game"},
							{name: "col", type: "string", defaultValue: "000000"},
							{name: "dur", type: "number", defaultValue: 0.5}
						]
					},
				]
	...
	{
		var spawnX = camEditor.scroll.x + 380 + lastSpawnPos.x;
		var spawnY = camEditor.scroll.y + 220 + lastSpawnPos.y;
		var block = new Block(spawnX, spawnY, data);
	...
	override function update(elapsed:Float)
	{
		...
		if (maxH < 0)
			maxH = 0;
		var pointerX = getPointerScreenX();
		var pointerY = getPointerScreenY();
		var pointerJustPressed = isPointerJustPressed();
		var pointerJustReleased = isPointerJustReleased();
		var pointerPressed = isPointerPressed();
		...
		if (isDraggingScroll && maxH > 0 && pointerPressed)
		{
			var thumbY = pointerY - scrollDragOffset;
		...
		else if (pointerX < 320)
		{
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				if (camSidebar.scroll.y < 0)
					camSidebar.scroll.y = 0;
				if (camSidebar.scroll.y > maxH)
					camSidebar.scroll.y = maxH;
			}
			#if android
			var touch = getPrimaryTouch();
			if (touch != null && touch.pressed)
			{
				var dy = sidebarDragLastY - pointerY;
				camSidebar.scroll.y += dy;
				if (camSidebar.scroll.y < 0)
					camSidebar.scroll.y = 0;
				if (camSidebar.scroll.y > maxH)
					camSidebar.scroll.y = maxH;
	...
	function handleTooltips()
	{
		...
		if (hoveredDesc != "")
		{
			...
			if (tx + 300 > FlxG.width)
				tx = FlxG.width - 310;
			if (ty + 50 > FlxG.height)
				ty = FlxG.height - 60;
	...
	function handleInput(elapsed:Float)
	{
		...
		if (inputFocused)
			return; // Disable shortcuts if typing
		...
		if (touch != null)
		{
			if (touch.justPressed && touch.screenX >= 320 && !isDragging)
			{
				var worldPos = getPointerWorldPosition(camEditor);
			...
			{
				var dx = panStart.x - touch.screenX;
				var dy = panStart.y - touch.screenY;
				camEditor.scroll.x += dx / zoomLevel;
				camEditor.scroll.y += dy / zoomLevel;
				panStart.set(touch.screenX, touch.screenY);
			}
	...
	function handleMouseDrag()
	{
		...
		var worldMouse = getPointerWorldPosition(camEditor);
		var pointerX = getPointerScreenX();
		var pointerY = getPointerScreenY();
		var pointerJustPressed = isPointerJustPressed();
		var pointerJustReleased = isPointerJustReleased();
		if (!isDragging && pointerJustPressed && pointerX < 320)
			return;
		...
		if (pointerJustPressed)
		{
			...
			while (i >= 0)
			{
				...
				for (input in block.inputFields)
				{
					...
					if (worldMouse.x >= input.bg.x
						&& worldMouse.x <= input.bg.x + input.width
						&& worldMouse.y >= input.bg.y
						&& worldMouse.y <= input.bg.y + input.height)
					{
						clickedInput = true;
						break;
					}
				...
				if (!clickedInput && block != null
					&& worldMouse.x >= block.x
					&& worldMouse.x <= block.x + block.width
					&& worldMouse.y >= block.y
					&& worldMouse.y <= block.y + block.height)
				{
					draggingBlock = block;
					...
					dragOffset.set(pointerX - draggingBlock.x, pointerY - draggingBlock.y);
					isDragging = true;
		...
		if (pointerJustReleased)
		{
			if (isDragging && draggingBlock != null)
			{
				...
				// Restore to Editor
				var mouseWorld = getPointerWorldPosition(camEditor);
				...
				if (screenPos.x < 320)
				{
					FlxTween.tween(draggingBlock.scale, {x: 0, y: 0}, 0.2, {
						ease: FlxEase.backIn,
	...
	{
		var px = getPointerScreenX();
		var py = getPointerScreenY();
	...
	function isPointerOverBlock(worldPos:FlxPoint):Bool
	{
		for (block in blockContainer.members)
		{
			if (block == null)
				continue;
			if (worldPos.x >= block.x && worldPos.x <= block.x + block.width && worldPos.y >= block.y && worldPos.y <= block.y + block.height)
				return true;
		}
		return false;
	}
	...
	function checkSnapping(current:Block)
	{
		var snapDist = 30.0;
		var worldPos = getPointerWorldPosition(camEditor);
		var proposedX = worldPos.x - (dragOffset.x / zoomLevel);
		var proposedY = worldPos.y - (dragOffset.y / zoomLevel);
		var bestDist = snapDist;
		for (other in blockContainer.members)
		{
			if (other == null || other == current || other.isReporter)
				continue;
			if (other.nextBlock != null)
				continue;
			var targetY = other.y + other.height;
			var dist = new FlxPoint(proposedX, proposedY).distanceTo(new FlxPoint(other.x, targetY));
			if (dist < bestDist)
			{
				bestDist = dist;
			}
		...
		for (other in blockContainer.members)
		{
			...
			for (input in other.inputFields)
			{
				if (input.attachedBlock != null)
					continue;
				var dist = new FlxPoint(proposedX, proposedY).distanceTo(new FlxPoint(input.bg.x, input.bg.y));
				if (dist < bestDist)
					bestDist = dist;
			}
		}
	...
	function applySnapping(current:Block)
	{
		...
		for (other in blockContainer.members)
		{
			if (other == current || other == null || other.isReporter)
				continue;
			if (other.nextBlock != null)
				continue;
	...
	function generateBlockCode(block:Block):String
	{
		...
		var q = function(val:Dynamic):String
		{
			...
			if (s.indexOf("(") != -1 || Std.parseFloat(s) != Math.NaN || s == "true" || s == "false")
				return s;
			return "'" + s + "'";
		};
		...
		switch (block.blockData.type)
		{
			...
			case "if":
				return "if " + v(p(0, "true")) + " then";
			case "elseif":
				return "elseif " + v(p(0, "true")) + " then";
			case "else":
				return "else";
			case "end":
				return "end";
			case "wait":
				return "runTimer('timer', " + v(p(0, 1)) + ")";
			case "debugPrint":
				return "debugPrint(" + q(p(0, "hello")) + ")";
			case "close":
				return "close(true)";

			// Operators
			case "add":
				return "(" + v(p(0, 0)) + " + " + v(p(1, 0)) + ")";
			case "sub":
				return "(" + v(p(0, 0)) + " - " + v(p(1, 0)) + ")";
			case "mul":
				return "(" + v(p(0, 0)) + " * " + v(p(1, 0)) + ")";
			...
			case "lt":
				return "(" + v(p(0, 0)) + " < " + v(p(1, 0)) + ")";
			case "and":
				return "(" + v(p(0, "true")) + " and " + v(p(1, "true")) + ")";
			case "or":
				return "(" + v(p(0, "true")) + " or " + v(p(1, "true")) + ")";
			case "not":
				return "(not " + v(p(0, "true")) + ")";
			case "join":
				return "(" + q(p(0, "a")) + " .. " + q(p(1, "b")) + ")";

			// Sensing
			case "keyJustPressed":
				return "keyJustPressed(" + q(p(0, "space")) + ")";
			case "keyPressed":
				return "keyPressed(" + q(p(0, "space")) + ")";
			case "curStep":
				return "curStep";
			case "curBeat":
				return "curBeat";
			...
			case "getPixelColor":
				return "getPixelColor(" + q(p(0, "obj")) + ", " + v(p(1, 0)) + ", " + v(p(2, 0)) + ")";

			case "setProperty":
				return "setProperty(" + q(p(0, "tag")) + ", " + v(p(1, "val")) + ")";
			case "getProperty":
				return "getProperty(" + q(p(0, "tag")) + ")";
			case "makeLuaSprite":
				return "makeLuaSprite(" + q(p(0, "spr")) + ", " + q(p(1, "img")) + ", " + v(p(2, 0)) + ", " + v(p(3, 0)) + ")";
			case "makeAnimatedLuaSprite":
				return "makeAnimatedLuaSprite(" + q(p(0, "spr")) + ", " + q(p(1, "img")) + ", " + v(p(2, 0)) + ", " + v(p(3, 0)) + ")";
			case "addLuaSprite":
				return "addLuaSprite(" + q(p(0, "spr")) + ", " + v(p(1, "false")) + ")";
			case "scaleObject":
				return "scaleObject(" + q(p(0, "tag")) + ", " + v(p(1, 1)) + ", " + v(p(2, 1)) + ")";
			case "setObjectCamera":
				return "setObjectCamera(" + q(p(0, "tag")) + ", " + q(p(1, "hud")) + ")";
			case "screenCenter":
				return "screenCenter(" + q(p(0, "tag")) + ")";

			case "playSound":
				return "playSound(" + q(p(0, "snd")) + ", " + v(p(1, 1)) + ")";
			case "playMusic":
				return "playMusic(" + q(p(0, "snd")) + ", " + v(p(1, 1)) + ", " + v(p(2, "true")) + ")";
			case "pauseSound":
				return "pauseSound(" + q(p(0, "snd")) + ")";
			case "stopSound":
				return "stopSound(" + q(p(0, "snd")) + ")";
			...
			case "setHealth":
				return "setProperty('health', " + v(p(0, 1)) + ")";
			case "addHealth":
				return "addHealth(" + v(p(0, 0.1)) + ")";
			case "cameraShake":
				return "cameraShake(" + q(p(0, "game")) + ", " + v(p(1, 0.05)) + ", " + v(p(2, 0.5)) + ")";
			case "cameraFlash":
				return "cameraFlash(" + q(p(0, "game")) + ", " + q(p(1, "FFFFFF")) + ", " + v(p(2, 0.5)) + ")";
			case "cameraFade":
				return "cameraFade(" + q(p(0, "game")) + ", " + q(p(1, "000000")) + ", " + v(p(2, 0.5)) + ")";

			default:
				return "-- " + block.blockData.label;
		}
	}
...
class Block extends FlxSprite
{
	...
	public function redrawBlock()
	{
		...
		var darkColor = color.getDarkened(0.4);
		var lightColor = color.getLightened(0.15);
		...
		else
		{
			...
			pixels.fillRect(new Rectangle(20, h - 4, 20, 4), 0xFF101014);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (!isDragging)
		{
			var target = (isMouseOver(camera) && parentInput == null && prevBlock == null) ? 1.05 : 1.0;
			scale.x = FlxMath.lerp(scale.x, target, elapsed * 8);
			scale.y = FlxMath.lerp(scale.y, target, elapsed * 8);
		}
	}
...
class InputField
{
	...
	public function new(x:Float, y:Float, w:Float, h:Float, defaultVal:Dynamic, type:String, name:String = "")
	{
		...
		bg = new FlxSprite(x, y).makeGraphic(Std.int(w), Std.int(h), BlockCodeEditorState.COLOR_INPUT_BG);

		text = new FlxText(x + 2, y + 4, w - 4, Std.string(value), 14);
		text.setFormat(Paths.font("vcr.ttf"), 14, BlockCodeEditorState.COLOR_INPUT_TEXT, CENTER);

		placeholderText = new FlxText(x + 2, y + 4, w - 4, placeholder, 14);
		placeholderText.setFormat(Paths.font("vcr.ttf"), 14, BlockCodeEditorState.COLOR_INPUT_PLACEHOLDER, CENTER);
	...
	public function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed || (touch != null && touch.justPressed))
		{
			...
			var mPos = new FlxPoint(FlxG.mouse.screenX, FlxG.mouse.screenY);
			if (touch != null)
			{
				mPos.x = cam.scroll.x + touch.screenX / cam.zoom;
				mPos.y = cam.scroll.y + touch.screenY / cam.zoom;
			}
			else
			{
				mPos = FlxG.mouse.getWorldPosition(cam);
			}

			if (mPos.x >= bg.x && mPos.x <= bg.x + width && mPos.y >= bg.y && mPos.y <= bg.y + height)
			{
				isFocused = true;
				bg.color = BlockCodeEditorState.COLOR_INPUT_FOCUS;
			}
			else
			{
				isFocused = false;
				bg.color = BlockCodeEditorState.COLOR_INPUT_BG;
			}
		}
