		...
		if (!blockInput && !animationDropDown.dropPanel.visible)
		{
				...
				&& character.dialogueAnimations.exists(curSelectedAnim))
			{
				...
				var controlArrayLoop:Array<Bool> = [
					FlxG.keys.justPressed.A  #if android || _touchpad.buttonLeft.justPressed #end,
					FlxG.keys.justPressed.W    #if android || _touchpad.buttonUp.justPressed #end,
					FlxG.keys.justPressed.D #if android || _touchpad.buttonRight.justPressed #end,
					FlxG.keys.justPressed.S  #if android || _touchpad.buttonDown.justPressed #end];
