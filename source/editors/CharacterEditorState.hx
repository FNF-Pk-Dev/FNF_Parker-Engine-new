...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if android
		addTouchPad("FULL", "CHARACTER_EDITOR");
		addPadCamera();
		#end
	...
	function reloadCharacterImage()
	{
		...
		var isAtlas:Bool = Paths.fileExists('images/' + char.imageFile + '/Animation.json', TEXT);
		if (isAtlas)
		{
		...
		if (char.animationsArray != null && char.animationsArray.length > 0)
		{
			for (anim in char.animationsArray)
			{
				...
				{
					char.animation.addByIndices(animAnim, isAtlas ? animName + '_' : animName, animIndices, "", animFps, animLoop);
				}
				else
				{
					char.animation.addByPrefix(animAnim, isAtlas ? animName + '_' : animName, animFps, animLoop);
				}
			}
		}
		else
	...
	override function update(elapsed:Float)
	{
		...
		if (!charDropDown.dropPanel.visible)
		{
			...
			if (FlxG.keys.justPressed.R #if android || _touchpad.buttonZ.justPressed #end)
			{
				FlxG.camera.zoom = 1;
			}

			if (FlxG.keys.pressed.E #if android || _touchpad.buttonX.pressed #end && FlxG.camera.zoom < 3)
			{
				FlxG.camera.zoom += elapsed * FlxG.camera.zoom;
				if (FlxG.camera.zoom > 3)
					FlxG.camera.zoom = 3;
			}
			if (FlxG.keys.pressed.Q #if android || _touchpad.buttonY.pressed #end && FlxG.camera.zoom > 0.1)
			{
				FlxG.camera.zoom -= elapsed * FlxG.camera.zoom;
			...
			{
				if (FlxG.keys.justPressed.W #if android || _touchpad.buttonV.justPressed #end)
				{
					curAnim -= 1;
				}

				if (FlxG.keys.justPressed.S #if android || _touchpad.buttonD.justPressed #end)
				{
					curAnim += 1;
				}
				...
				if (FlxG.keys.justPressed.S #if android || _touchpad.buttonD.justPressed #end || FlxG.keys.justPressed.W #if android
					|| _touchpad.buttonV.justPressed #end
					|| FlxG.keys.justPressed.SPACE)
				{
					char.playAnim(char.animationsArray[curAnim].anim, true);
					genBoyOffsets();
				}
				if (FlxG.keys.justPressed.T #if android || _touchpad.buttonA.justPressed #end)
				{
				...
				var controlArray:Array<Bool> = [
					 FlxG.keys.justPressed.LEFT  #if android || _touchpad.buttonLeft.justPressed #end,
					FlxG.keys.justPressed.RIGHT #if android || _touchpad.buttonRight.justPressed #end,
					   FlxG.keys.justPressed.UP    #if android || _touchpad.buttonUp.justPressed #end,
					 FlxG.keys.justPressed.DOWN  #if android || _touchpad.buttonDown.justPressed #end];

				for (i in 0...controlArray.length)
				{
					if (controlArray[i])
					{
						var holdShift = FlxG.keys.pressed.SHIFT #if android || _touchpad.buttonB.pressed #end;
						var multiplier = 1;
						if (holdShift)
							multiplier = 10;
