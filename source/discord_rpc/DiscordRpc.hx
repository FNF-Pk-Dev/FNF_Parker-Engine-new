...
class DiscordRpc
{
	...
	{
		DiscordRpcExterns.setPresence(options.state, options.details, options.startTimestamp, options.endTimestamp, options.largeImageKey,
			options.largeImageText, options.smallImageKey, options.smallImageText, options.partyID, options.partySize, options.partyMax, options.matchSecret,
			options.joinSecret, options.spectateSecret, options.instance);
	}
...
#if !display
@:build(linc.Linc.touch()) @:build(linc.Linc.xml('discord_rpc'))
#end
private extern class DiscordRpcExterns
{
	@:native('linc::discord_rpc::init')
	private static function _init(clientID:String, steamAppID:String, onReady:VoidCallback, onDisconnected:ErrorCallback, onError:ErrorCallback,
		onJoin:SecretCallback, onSpectate:SecretCallback, onRequest:RequestCallback):Void;

	static inline function init(clientID:String, ?steamAppID:String):Void
	{
		_init(clientID, steamAppID, Function.fromStaticFunction(_onReady), Function.fromStaticFunction(_onDisconnected),
			Function.fromStaticFunction(_onError), Function.fromStaticFunction(_onJoin), Function.fromStaticFunction(_onSpectate),
	...
	public static function setPresence(state:String, details:String, startTimestamp:cpp.Int64, endTimestamp:cpp.Int64, largeImageKey:String,
		largeImageText:String, smallImageKey:String, smallImageText:String, partyID:String, partySize:Int, partyMax:Int, matchSecret:String,
		joinSecret:String, spectateSecret:String, instance:cpp.Int8):Void;
