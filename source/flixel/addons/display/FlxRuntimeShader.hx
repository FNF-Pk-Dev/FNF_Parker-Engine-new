	...
	private override function __initGL():Void
	{
		...
		if (__context != null && program == null)
		{
			...
			else
			{
				...
				{
					// lime's Log.error throws when a shader fails to compile; a broken mod shader
					// must not kill the whole game, so fall back to the default passthrough shader
					trace('FlxRuntimeShader: shader failed to compile, falling back to default. '
						+ Std.string(e).substr(0, 600));

					var fallbackVertex = StringTools.replace(processVertexSource(DEFAULT_VERTEX_SOURCE), PRAGMA_PRECISION, precisionHeaders);
					fallbackVertex = StringTools.replace(fallbackVertex, PRAGMA_VERSION, versionHeader);
					var fallbackFragment = StringTools.replace(processFragmentSource(DEFAULT_FRAGMENT_SOURCE), PRAGMA_PRECISION, precisionHeaders);
					fallbackFragment = StringTools.replace(fallbackFragment, PRAGMA_VERSION, versionHeader);
					program.__glProgram = __createGLProgram(fallbackVertex, fallbackFragment);
				}
