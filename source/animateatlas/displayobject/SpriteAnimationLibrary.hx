		...
		for (layerData in layerDates)
		{
			...
			for (frame in frames)
			{
				...
				for (e in 0...elements.length)
				{
					...
					{
						// short-key (Animate 2019+) sprites carry their placement in Matrix3D
						var spriteMatrix:Matrix3DData = element.ATLAS_SPRITE_instance.Matrix3D;
						element = elements[e] = {
							SYMBOL_Instance: {
								SYMBOL_name: BITMAP_SYMBOL_NAME,
								...
								Matrix3D: spriteMatrix != null ? spriteMatrix : STD_MATRIX3D_DATA
							}
						};
					}
				}
