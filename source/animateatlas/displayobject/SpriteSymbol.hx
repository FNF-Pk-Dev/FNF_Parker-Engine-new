...
class SpriteSymbol extends Sprite
{
	...
	private function updateLayer(layerIndex:Int):Void
	{
		...
		for (i in 0...numElements)
		{
			...
			else if (newSymbol._composedFrame != newSymbol._currentFrame)
			{
				// A fresh/pooled movie clip starts with an empty display list: compose it
				// at its current frame or the layer renders nothing
				newSymbol.currentFrame = newSymbol._currentFrame;
			}
		}
