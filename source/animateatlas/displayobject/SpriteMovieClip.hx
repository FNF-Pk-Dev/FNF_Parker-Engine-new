...
class SpriteMovieClip extends Sprite
{
	...
	/** Advances the whole clip (and nested clips) by whole frames, regardless of elapsed time. */
	public function stepFrames(n:Int = 1):Void
	{
		for (i in 0...n)
		{
			symbol.nextFrame();
	...
	/** Advances only nested movie clips, for when the root frame is set explicitly. */
	public function stepNested(direction:Int = 1):Void
	{
		symbol.moveMovieclip_MovieClips(direction);
	}
