...
- `mod_patches/LockedDestiny/` is a tracked overlay for the locally installed mod in `export/release/windows/bin/mods/LockedDestiny/`. Copy its files into an existing complete mod; it does not include art/audio and is not a standalone mod. Keep the overlay and installed files in sync when fixing this mod. Its SBF/SBFG JSON offsets compensate for the baked Animate canvas origins (-145, -212) / (-148, -225); changing those atlases requires recalibrating the offsets. `NeurosesCharacters.lua` uses the initial pose's trimmed dimensions for camera hitboxes, preserving the render origin/scale. The opening `goodapple` flag hides the TV, TV static, central character and shadows until step 288 (21.6 s at 200 BPM).
