...
`Paths.font()` resolves font files, including extensionless mod keys such as `SBF` (`.ttf`/`.otf`). `Paths.fontName()` loads and **registers** a native font with `openfl.text.Font.registerFont()` before returning its internal family name, caching registrations by the resolved absolute file path. `Font.fromFile()` alone does not register it: returning that unregistered family makes FlxText silently fall back to a system font. Use `fontName()` where a TextFormat/FlxText requires a family rather than a file path.
...
- `mod_patches/LockedDestiny/` is a tracked overlay for the locally installed mod in `export/release/windows/bin/mods/LockedDestiny/`. Copy its files into an existing complete mod; it does not include art/audio and is not a standalone mod. Keep the overlay and installed files in sync when fixing this mod. Its SBF/SBFG JSON files restore the original animation offsets: `AtlasFrameMaker` / `Character` now handle baked canvas translations centrally, so do not reintroduce manual offsets or scripted width/height compensation. `NeurosesCharacters.lua` gates `gfGroup.visible` while `goodapple` is true (including character swaps); the opening flag clears at step 288 (21.6 s at 200 BPM). `tvN.lua` places later scene backgrounds above TVS/gfGroup and below dad/boyfriend, since ES add-behind normally inserts before gfGroup.
...
**ES step dispatch.** `ESStepEvents` records the dispatch in which each due step fired. Every `stepEvent` handler for that step may run during the same `onEventSet` pass, while later passes do not replay it. A per-script `Map<Int, Bool>` is insufficient: Neuroses uses two handlers at step 1567, and dropping the second leaves glitch enabled throughout the red tunnel. `FunkinLua.call` begins the dispatch before entering Lua. Run `tests/check-es-steps.ps1` for the standalone scheduler regression check. ES `videoPlay` resolves shared tagged objects, so a song script can start a video created by its stage.

**ES scale tweens accept two signatures.** `doTweenScale(tag, object, scale, duration, ease)` scales both axes; `(tag, object, scaleX, scaleY, duration, ease)` keeps separate axes. The fifth argument must accept either a duration or an ease string. Treating it as a required Float leaves Neuroses's `TX2` at its initial 29x scale and also breaks the tunnel's `spikes` tween.

**Custom death may veto the default from either runtime.** `PlayState.doDeathCheck()` opens `GameOverSubstate` only when both Lua and the other script runtimes return something other than `Function_Stop` (AND, not OR). Locked Destiny's `customDeath.lua` owns its `CustomSubstate`, keeps the parent updating for its sprites/timers, and pauses the song audio explicitly. Its reflection calls use `backend.songs.Conductor`, not the nonexistent root `Conductor` class. AutoText observes `autoTextDisabled` and cancels its outstanding typing/fades on death.

**Lua boolean flags require explicit checks.** The installed `llua.Convert.anon_function` maps a null callback result to numeric `0`; `FunkinLua.set()` binds functions through that bridge. An unset `getVar()` can therefore yield `0`, which is truthy in Lua. AutoText initializes `autoTextDisabled` to `false` in `onLoad` and checks `== true` before disabling its step events or typing. Preserve both checks and model the native zero result in `tests/LockedDestinyScriptsTest.lua`; a mock returning nil hides this regression.
...
**Shadow choreography belongs to the mod.** `PlayState` has no `*CopyFrames` fields and `ESCompat` no longer automatically copies every `makeChar()` object on a side. Locked Destiny's `scripts/NeurosesCharacters.lua` explicitly maps `ShadowBF`/`ShadowBF2` to BF and `ShadowSBF`/`SBFG` to dad. It disables autonomous updates on the followers and copies the animation name and frame in `onUpdatePost`, after the real characters have updated. Preserve each shadow's own position, offset, flip, scale and alpha. Missing animations are skipped; missed sings fall back to the corresponding regular sing.

Locked Destiny's `Fly.lua` restores dad's authored position from `dadGroup` plus `dad.positionArray` immediately on `Change Character`. Cached characters retain choreography coordinates, and a delayed timer lets the old floating base overwrite a newly selected character before it is sampled. Only the explicit step-1800 transition adopts the current scene position as a new floating base. The same mod's HUD script keeps its drawn icons antialiased and upright; the general `HealthIcon` pixel exception recognizes a `pixel` name component, including `bf-pixel-alt`.

The mod's `Functions.lua` also restores BF's base tint and RGB transform on player `Change Character` events once `goodapple` is off. The step-1560 silhouette leaves cached BF2 black: the chart selects BFCR before step 1568 restores the active character, then reuses BF2 at 155958.75 ms for the ending. Reset on the swap itself to avoid a black frame; preserve the character shader and dad's intentional ending fade. This is song choreography, so keep it in the mod rather than resetting every engine character on every swap.

**Event videos are tweenable objects.** `PlayState.startVideo(name, ...)` registers the returned `VideoSprite` group in `variables` under `name` (case-sensitive). A `Play Video` event for `IntroN` therefore supports `doTweenAlpha('fade', 'IntroN', 0, 0.5)` and `FlxTween.tween(game.variables.get('IntroN'), {alpha: 0}, 0.5)`. The group explicitly renders on `camVideo`, so camera alpha also works even when a debug overlay appends another camera. `VideoSprite.destroy()` is idempotent, cancels object tweens and releases the name only if it still points to that group.
...
**Lua sound playback must resolve an asset before touching the sound pool.** `playSound` tries `Paths.sound()` then `Paths.music()` for ES scripts that play death/pause music on independent tagged channels. A missing asset is reported before stopping/replacing any existing tag. Never pass null to `FlxG.sound.play`: Flixel's `loadEmbedded(null)` skips reinitialization of a recycled sound and `loadHelper` can dereference its destroyed transform while setting volume. Completion captures the owning sound map and checks the instance, rather than looking up whatever menu/game registry is current later. Run `tests/check-lua-sound.ps1` to exercise the production callback with a recording audio backend. Locked Destiny's pause script only loops its own `pauseMusic` tag while its substate is open; the package lacks `lullabyPause`, so it falls back to the configured engine pause music (including None).
