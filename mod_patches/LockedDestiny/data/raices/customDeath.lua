...
local Assets = {
	...
	sounds = {
		loss = 'fnf_loss_sfx-pixel',
		music = 'gameOver-pixel',
		confirm = 'gameOverEnd-pixel'
	}
}
...
function onGameOver()
	if not getVar('canDie') then return Function_Stop
	end
...
function onCustomSubstateCreate(tag)
	...
	addOffset('dead','loop',-27,-11)
	addOffset('dead','confirm',-27,-11)
	set('dead.antialiasing', false)
	set('dead.scale.x', 5.5)
	set('dead.scale.y', 5.5)
